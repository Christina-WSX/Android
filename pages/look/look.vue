	...
	<view>
		<view class="grace-header-cate">
            <scroll-view class="grace-tab-title grace-center" scroll-x="true" id="grace-tab-title">
                <view v-for="(cate, index) in catemeals" :key="index" :data-cateid="cate.cateId" :data-index="index" :class="[cateCurrentindex == index ? 'grace-tab-current' : '']" @tap="tabChange">{{cate.name}}</view>
            </scroll-view>
        </view>
        ...
        <view class="grace-news-list">
            <block v-for="(item, index) in artList" :key="index">
                <!-- 一张图 -->
                <navigator v-if="item.art_content.length < 3" open-type="navigate" :url="'../infer/infer?artid='+item.art_id">
                ...
                <!-- 三张图 -->
                <navigator v-if="item.art_content.length >= 3" :url="'../infer/infer?artid='+item.art_id">
                    <view class="grace-news-list-img-news">
                        <view class="grace-news-list-title-main">{{item.art_title}}</view>
...
export default {
            ...
            success: res => {
                if(res.data.status == 'ok'){
                    ...
                    for(var k in catemeals){
                        _self.catemeals.push({cateId : k, name : catemeals[k]});
                    }
                }
            }
        });
	...
	methods: {
		...
		getNewsList : function(){
			...
			uni.request({
                url: this.apiServer + 'arts&m=getList&cate='+cate+'&page='+page,
                method: 'GET',
                success: res => {
						...
						for(var i = 0; i < newsList.length; i++){
                            ...
                            for(var ii = 0; ii < content.length; ii++){
                                if(content[ii].type == 'image'){
                                    img.push(content[ii].content);
                                }
                            }
                            newsList[i].art_content = img;
                        }
